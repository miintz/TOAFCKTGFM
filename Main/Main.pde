...
void mouseClicked()
{
  ...
  if(MODE == "SYNC")
  {   
    syncer.syncMouseClick();
  }
}

void keyPressed()
{
    if(SYNC_MODE == "register")
    {
...
String SYNC_INST = "none";
boolean INTRO_DRAWN = false;

ArrayList<Object[]> ListOfInstruments = new ArrayList<Object[]>(); 
...
public class Syncer
{
  ...
  public void draw()
  {
    ...
    if (SYNC_MODE == "intro")
    {
      ...
      textSize(16);
      text("Welcome to the sync mode. Select below what you wish to sync.", 25, 200);

      textSize(32);
      
      text("Lyrics", 200, 400);
      text("Guitars", 200, 500);
      text("Drums", 200, 600);
      text("Other", 200, 700);
    ...
    if(SYNC_MODE == "register")
    {
      ...
      text("Syncing " + SYNC_INST + ". Press the keys you wish to use.",200,100);      
      ...
      {
         syncinstr = ListOfInstruments.get(i);
       
         ArrayList<String> list = (ArrayList<String>)syncinstr[1];
         String na = (String)syncinstr[0];
         
         if(na == SYNC_INST)
         {
           for(int y = 0; y > list.size(); i++)
             text(list.get(y), 100, (100 * i) / 2);                
         }    
      }
      
      //String[] list-o-keys = ArrayList.get(S      
    }
  }

  public void syncMouseClick()
  {
    if(SYNC_MODE == "intro" && INTRO_DRAWN) 
    {
      if((mouseX < 300 && mouseY < 400) && mouseX > 200 && mouseY > 300)      
        SYNC_INST = "lyrics";
      
      if((mouseX < 300 && mouseY < 500) && mouseX > 200 && mouseY > 400)
        SYNC_INST = "guitars";
      
      if((mouseX < 300 && mouseY < 600) && mouseX > 200 && mouseY > 500)  
        SYNC_INST = "drums";
      
      if((mouseX < 300 && mouseY < 700) && mouseX > 200 && mouseY > 600) 
        SYNC_INST = "other";
      ...
      ins[0] = SYNC_INST;
      ins[1] = new ArrayList<String>();
      
      ListOfInstruments.add(ins);    
    }
    
    if(SYNC_MODE == "register")
    {
      //een 'volgende' knop ofzo.
  ...
  public void syncKeyPressed()
  {
    if(SYNC_MODE == "register")
    {
      ...
      int i = 0;
      for(i = 0; i < ListOfInstruments.size(); i++) 
      {         
         ins = ListOfInstruments.get(i);
         list = (ArrayList<String>)ins[1];
         if(ins[0] == SYNC_INST)
         {
